...
uint64_t pcm_get_timestamp(struct pcm* pcm, uint32_t sample_rate, unsigned int isOutput) {
    ...
    if (pcm == NULL) {
        printf("Error getting PCM timestamp, pcm is null");
        return 0;
    }
    if (pcm_get_htimestamp(pcm, &available, &timestamp) < 0) {
        printf("Error getting PCM timestamp!");
        return 0;
    }
    int frames = 0;
    if (isOutput) {
        frames = (int) (pcm_get_buffer_size(pcm) - available);
    } else {
                     ...
                     unsigned int flags, struct pcm_config *config)
{
    ...
    printf("%s(): flag = %x, card = %d, device = %d, fd = %d",
