...
// run process as root user in root group, check gid in /etc/group
#define AID_LOG 0
...
void parent(const char *tag, int seg_fault_on_exit, int parent_read) {
    ...
    while ((sz = read(parent_read, &buffer[b], sizeof(buffer) - 1 - b)) > 0) {
        ...
        for (b = 0; b < sz; b++) {
            if (buffer[b] == '\r') {
                ...
                if (should_log_line) {
                    ...
                    ALOG(LOG_INFO, "logwrapper", "%s", "[TRUNCATED]");
                }
        ...
        if (a == 0 && b == sizeof(buffer) - 1) {
            ...
            // Truncate the log line if its size is greater than 4096.
