...
static const TracingCategory k_categories[] = {
    ...
    { "meson_atrace", "Meson Kernel Atrace", 0, {
        {OPT,       "events/meson_atrace/enable"},
    } },
};
...
{
    std::ifstream clockFile((g_traceFolder + k_traceClockPath).c_str());
...
//{
//    std::string buf;
//    if (!ReadFileToString(g_traceFolder + k_ftraceFilterPath, &buf)) {
//         fprintf(stderr, "error opening %s: %s (%d)\n", k_ftraceFilterPath,
//            strerror(errno), errno);
//         return false;
//    }
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        ret = getopt_long(argc, argv, "a:b:cf:k:ns:t:zo:p",
