...
function send_command(command, callback,cgi)
{
...
function get_bt_list()
{
	...
	var wifi_json = send_command("",bt_list_init,"cgi-bin/bt.cgi");
}
...
$('#bt_btn').on('click', 'button', function(){
	send_command("", is_bt_ok, "cgi-bin/bt_con.cgi?index="+eval(this.name));
})
function is_bt_ok(){
...
{
	var wifi_list = null;
	var wifi_json = send_command("get_wifi_list", set_wifi_list_to_select);
	show_loading();
...
function set_wifi(ssid,pwd)
{
	var wifi_ssid = ssid;
	var wifi_pwd = pwd;
	var wifi_json = send_command("set_wifi&ssid="+wifi_ssid+"&"+"pwd="+wifi_pwd);
	show_loading();
...
function check_wifi_connected1() {
    show_loading();
    send_command("check_wifi_connected", handle_check_wifi1);
}
...
function check_wifi_connected() {
	...
	send_command("check_wifi_connected", handle_check_wifi);
}
...
{
	send_command("get_record_list", handle_record_list);
}

function get_record_dl_list() {
	send_command("get_record_dl_list", handle_record_dl_list);
}
...
function start_record() {
	...
	var channel = $('#select-ch option:selected').val();
	var rate = $('#select-rate option:selected').val();
	var bits = $('#select-bits option:selected').val();
	console.log(channel);
	console.log(rate);
	console.log(bits);
	send_command("start_record&filename="+input_filename+"&ch="+channel+"&rate="+rate+"&bits="+bits);
	//get_record_list();
}

function done_record() {
	send_command("done_record");
	get_record_list();
}

function delete_record() {
	var input_filename = document.getElementById("modal_filename").value;
	send_command("delete_record&filename="+input_filename);
	get_record_list();
}

function play_record() {
    var input_filename = document.getElementById("modal_filename").value;
    send_command("play_record&filename="+input_filename);
}
...
function set_spotify(){
	...
	send_command("set_spotify&username="+spotify_username+"&"+"spotify_pwd="+spotify_pwd+"&"+"spotify_dname="+spotify_dname);
...
function check_spotify(){
	send_command("check_spotify",handle_check);
}
...
function handle_check(command_xmlhttp){
	...
	if(response_check_date.length > 7){
		send_command("get_spo_info",handle_get_info);
	}
	else{
...
function system(){
	...
	send_command("get_deviceinfo",show_device_info);
}

function show_device_info(command_xmlhttp){

	send_command("runswupdate");
