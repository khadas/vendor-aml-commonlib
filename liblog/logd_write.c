...
#endif
static int __write_to_log_init(int log_id, struct iovec *vec, size_t nr);
static int (*write_to_log)(int log_id, struct iovec *vec, size_t nr) = __write_to_log_init;
#ifdef HAVE_PTHREADS
...
static int __write_to_log_null(int log_fd, struct iovec *vec, size_t nr)
{
    return -1;
}

static int __write_to_log_kernel(int log_id, struct iovec *vec, size_t nr)
{
    ssize_t ret;
    int log_fd;
...
int __android_log_buf_write(log_id_t bufID, int prio, const char *tag, const char *msg)
{
    struct iovec vec[3];
