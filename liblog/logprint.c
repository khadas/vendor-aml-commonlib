                                 ...
                                 AndroidLogEntry *entry)
{
    ...
    if (buf->len < 3) {
        ...
        // fprintf here causes more errors. Just fail silently.
    ...
    if (msgStart == -1) {
        // Malformed log message: Do the best that we can with it by
        // extracting the longest string of printable characters from it.
        ...
        for (index = 0; index < buf->len; ++index) {
            unsigned char c = buf->msg[index];
            // Regardless of its value, treat the last character as a terminator
            if (isprint(c) && index < buf->len - 1) {
                if (found_printable) {
                    last = index;
                } else {
                    first = last = index;
                ...
                if (found_printable) {
                    if (last - first + 1 > longest_len) {
                        longest_len = last - first + 1;
                        longest_start = first;
                        found_printable = 0;
                    }
                }
        ...
        buf->msg[longest_start + longest_len] = '\0';
        entry->priority = ANDROID_LOG_INFO;
        entry->tag = "?BadLog";
        entry->message = buf->msg + longest_start;
        entry->messageLen = longest_len;
        return 0;
    }
    if (msgEnd == -1) {
    ...
    int messageBufLen)
{
    ...
    if (inCount != 0) {
        // fprintf(stderr,
        //    "Warning: leftover binary log data (%lu bytes)\n", inCount);
    }
...
// Determine if this is this a secure build.
//
// TODO(ghines): This probably belongs somewhere else.
int is_secure_build() {
    const char* secure_user_build = getenv("SECURE_USER_BUILD");
    int secure_value;
    if (secure_user_build != NULL) {
      char* endptr;
      long secure_value = strtol(secure_user_build, &endptr, 10);
      if (*endptr == '\0') {
        // strtol succeeded, return true if it is set to 1.
        return (secure_value == 1);
      }
    }

    // Any errors should default to the more secure mode.
    return 1;
}

static void android_log_filter_url(char *buf) {
    // do nothing if it is non-secure build.
    if (!is_secure_build()) {
        return;
    }

    // Match a url as efficiently as possible.
    // More than one url may be present, special characters may be encoded,
    // and we should ignore case.
    char *ptr = buf;
    const char http_str[] = "http";
    while ((ptr=strcasestr(ptr, http_str)) != NULL) {
        ptr += strlen(http_str);
        ...
        // one colon (encoded or not)
        const char enc_colon_str[] = "%3A";
        if (*ptr == ':') {
            ptr++;
        } else if (!strncasecmp(ptr, enc_colon_str, strlen(enc_colon_str))) {
            ptr += strlen(enc_colon_str);
        } else {
           continue;
        }

        // two slashes (encoded or not)
        int i;
        const char enc_slash_str[] = "%2F";
        for (i = 0; i < 2; i++) {
            if (*ptr == '/') {
                ptr++;
            } else if (!strncasecmp(ptr, enc_slash_str, strlen(enc_slash_str))) {
                ptr += strlen(enc_slash_str);
            } else {
                continue;
            }
        }

        // We have found a url, now censor it.

        // Stop when we hit a terminator or null character.
        while (!strchr("\f\n\r\t\v ", *ptr)) {
            *ptr++ = 'X';
        }
